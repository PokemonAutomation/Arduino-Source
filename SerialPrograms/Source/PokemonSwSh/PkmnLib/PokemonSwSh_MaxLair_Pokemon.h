...
#ifndef _PokemonAutomation_PokemonSwSh_MaxLair_Pokemon_H
#define _PokemonAutomation_PokemonSwSh_MaxLair_Pokemon_H
...
public:
    ...
    bool has_type(Type theType) { return theType == m_type1 || theType == m_type2; }
...
#if 0
Pokemon *newPokemonFromJSON(std::string pokemon_name, uint16 pokemon_id, json base_data, bool is_legendary);
std::map<uint16, Pokemon *> get_all_rental_pokemon();
std::map<uint16, Pokemon *> get_all_boss_pokemon();

void map_moves_to_pokemon(std::map<uint16, Pokemon *> &pkmn_map, std::map<uint32, moves::Move *> &move_map);
void map_max_moves_to_pokemon(std::map<uint16, Pokemon *> &pkmn_map, std::map<uint32, moves::Move *> &move_map);
#endif
