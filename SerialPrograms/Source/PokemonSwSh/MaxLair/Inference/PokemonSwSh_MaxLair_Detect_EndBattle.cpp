...
bool PokemonCaughtMenuDetector::detect(const QImage& screen){
    ImageStats top_white = image_stats(extract_box_shallow(screen, m_top_white));
//    cout << top_white.average << ", " << top_white.stddev << endl;
    if (!is_solid(top_white, {0.316068, 0.341966, 0.341966})){
//        cout << "Failed: m_top_white" << endl;
        return false;
    }
    ImageStats caught_left = image_stats(extract_box_shallow(screen, m_caught_left));
    if (!is_black(caught_left)){
//        cout << "Failed: m_caught_left" << endl;
        return false;
    }
    ImageStats caught_right = image_stats(extract_box_shallow(screen, m_caught_right));
...
//        cout << "Bad left/right distance." << endl;
    ...
    ImageStats middle_pink = image_stats(extract_box_shallow(screen, m_middle_pink));
...
//        cout << "Failed: m_middle_pink" << endl;
    ...
    ImageStats bottom_white = image_stats(extract_box_shallow(screen, m_bottom_white));
//    cout << bottom_white.average << ", " << bottom_white.stddev << endl;
    if (!is_solid(bottom_white, {0.331264, 0.332167, 0.336569}, 0.1, 20)){
        return false;
    }
    ImageStats bottom_black = image_stats(extract_box_shallow(screen, m_bottom_black));
    if (!is_black(bottom_black)){
        return false;
    }
    ImageStats bottom_options = image_stats(extract_box_shallow(screen, m_bottom_options));
...
size_t count_catches(VideoOverlay& overlay, const QImage& screen){
    ...
    size_t count = 0;
    if (is_black(image_stats(extract_box_shallow(screen, box0)))){
        count++;
    }
    if (is_white(image_stats(extract_box_shallow(screen, box1)))){
        count++;
    }
    if (is_white(image_stats(extract_box_shallow(screen, box2)))){
        count++;
    }
    if (is_white(image_stats(extract_box_shallow(screen, box3)))){
        count++;
    }
