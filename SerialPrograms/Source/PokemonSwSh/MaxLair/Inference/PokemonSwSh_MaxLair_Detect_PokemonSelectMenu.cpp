...
//    cout << slot0.average << ", " << slot0.stddev << endl;
    ImageStats slot3 = image_stats(extract_box_shallow(screen, m_player3));
    if (slot3.stddev.sum() > 30) return 3;
    ImageStats slot2 = image_stats(extract_box_shallow(screen, m_player2));
    if (slot2.stddev.sum() > 30) return 2;
    ImageStats slot1 = image_stats(extract_box_shallow(screen, m_player1));
    if (slot1.stddev.sum() > 30) return 1;
    ImageStats slot0 = image_stats(extract_box_shallow(screen, m_player0));
