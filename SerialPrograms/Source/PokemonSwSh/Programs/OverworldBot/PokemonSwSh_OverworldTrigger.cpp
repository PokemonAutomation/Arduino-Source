...
{}
void OverworldTrigger::whistle(ControllerContext& context, bool rotate){
...
void OverworldTrigger_Whistle::run(ControllerContext& context){
...
{}
void OverworldTrigger_WhistleStaticAction::run(ControllerContext& context){
    if (m_whistle_first){
...
void OverworldTrigger_WhistleStaticAction::whistle_loop(ControllerContext& context){
    for (size_t c = 0; c < m_whistle_count; c++){
        whistle(context, !m_first_after_battle);
        m_first_after_battle = false;
    }
}
void OverworldTrigger_WhistleStaticAction::action_loop(ControllerContext& context){
    m_target_tracker.set_stop_on_target(true);
    context.wait_for_all_requests();
...
void OverworldTrigger_WhistleCircle::action(ControllerContext& context){
    circle_in_place(context, rand() % 2);
}
void OverworldTrigger_WhistleHorizontal::action(ControllerContext& context){
    move_in_line(context, true);
}
void OverworldTrigger_WhistleVertical::action(ControllerContext& context){
