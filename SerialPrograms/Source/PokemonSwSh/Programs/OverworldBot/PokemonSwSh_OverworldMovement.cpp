...
//    cout << "up" << endl;
    if (counter_clockwise){
        ...
        pbf_move_left_joystick(context, 0, 128, 128ms, 0ms);
...
void circle_in_place(ProControllerContext& context, bool counter_clockwise){
    if (counter_clockwise){
        pbf_move_left_joystick(context, 0, 128, 512ms, 0ms);    //  Correct for bias.
        ...
        pbf_move_left_joystick(context, 0, 128, 256ms, 0ms);
        ...
        pbf_move_left_joystick(context, 255, 128, 256ms, 0ms);
        pbf_move_left_joystick(context, 255, 255, 256ms, 0ms);
        pbf_move_left_joystick(context, 0, 128, 128ms, 0ms);    //  Correct for bias.
    }
}
void move_in_line(ProControllerContext& context, bool horizontal){
    if (horizontal){
        pbf_move_left_joystick(context, 0, 128, 1024ms, 256ms);
