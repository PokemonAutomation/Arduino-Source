...
LobbyDoneConnecting::LobbyDoneConnecting(VideoOverlay& overlay)
    : m_box(overlay, 0.600, 0.820, 0.080, 0.100)
{}
bool LobbyDoneConnecting::on_frame(
    const QImage& frame,
    std::chrono::system_clock::time_point timestamp
){
    return detect(frame);
...
LobbyAllReadyDetector::LobbyAllReadyDetector(VideoOverlay& overlay, size_t consoles)
    : m_consoles(consoles)
    , m_checkbox0(overlay, 0.669, 0.337 + 0.0775*0, 0.034, 0.06)
    , m_checkbox1(overlay, 0.669, 0.337 + 0.0775*1, 0.034, 0.06)
    , m_checkbox2(overlay, 0.669, 0.337 + 0.0775*2, 0.034, 0.06)
    , m_checkbox3(overlay, 0.669, 0.337 + 0.0775*3, 0.034, 0.06)
{}
bool LobbyAllReadyDetector::on_frame(
    const QImage& frame,
    std::chrono::system_clock::time_point timestamp
){
    return detect(frame);
...
bool LobbyAllReadyDetector::detect(const QImage& screen){
    size_t ready = 0;
//    ImageStats stats0 = pixel_stats(extract_box(screen, m_checkbox0));
