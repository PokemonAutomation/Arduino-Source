...
#include "PokemonSV_AreaZyroSkyDetector.h"
...
bool AreaZyroSkyDetector::detect(Kernels::Waterfill::WaterfillObject& object, const ImageViewRGB32& screen) const{
...
bool AreaZyroSkyDetector::detect(const ImageViewRGB32& screen) const{
    using namespace Kernels::Waterfill;
...
AreaZyroSkyTracker::AreaZyroSkyTracker(VideoOverlay& overlay)
    : VisualInferenceCallback("AreaZyroSkyTracker")
    , m_overlay(overlay)
{}

bool AreaZyroSkyTracker::sky_location(double& x, double& y) const{
    SpinLock m_lock;
    if (!m_box){
        return false;
    }
...
void AreaZyroSkyTracker::make_overlays(VideoOverlaySet& items) const{}
bool AreaZyroSkyTracker::process_frame(const ImageViewRGB32& frame, WallClock timestamp){
    using namespace Kernels::Waterfill;
...
void find_and_center_on_sky(
    ...
    AreaZyroSkyTracker sky_tracker(console);
