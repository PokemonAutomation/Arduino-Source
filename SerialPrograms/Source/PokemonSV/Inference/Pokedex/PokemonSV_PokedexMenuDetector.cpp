...
const ImageMatch::ExactImageMatcher& RECENTLY_BATTLED_CROPPED(){
...
//    cropped.save("test.png");
    double rmsd = RECENTLY_BATTLED_CROPPED().rmsd(cropped);
//    cout << rmsd << endl;
    return rmsd < 120;
}
...
PokedexMenuDetector::PokedexMenuDetector(
    Color color,
    ...
    , m_box(box)
{}
void PokedexMenuDetector::make_overlays(VideoOverlaySet& items) const{
    items.add(m_color, m_box);
}
bool PokedexMenuDetector::detect(const ImageViewRGB32& screen) const{
    using namespace Kernels::Waterfill;
...
PokedexMenuWatcher::PokedexMenuWatcher(
    Logger& logger,
    Color color, bool trigger_if_detected,
    std::function<void()> on_kill_callback,
    std::chrono::milliseconds duration
)
     : DetectorToFinder("PokedexMenuWatcher", duration, color)
     , m_logger(logger)
...
{}
bool PokedexMenuWatcher::process_frame(const ImageViewRGB32& frame, WallClock timestamp){
