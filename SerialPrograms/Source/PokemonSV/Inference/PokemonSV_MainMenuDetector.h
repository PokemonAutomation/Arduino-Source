...
public:
    ...
    std::pair<MenuSide, int> side(const ImageViewRGB32& screen) const;
...
class MainMenuFinder : public DetectorToFinder_ConsecutiveDebounce<MainMenuDetector>{
public:
    MainMenuFinder(Color color = COLOR_RED)
         : DetectorToFinder_ConsecutiveDebounce("MenuDetector", 5, color)
    {}
};
