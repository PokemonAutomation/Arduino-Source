...
void checkpoint_21(
    ...
    [&](size_t attempt_number){         
        ...
        // turn left
        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NO_MARKER, 0, 128, 400ms);
        // move forward
        pbf_move_left_joystick(context, {0, +1}, 800ms, 800ms);

        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 0, 200, 560ms);
        pbf_move_left_joystick(context, {0, +1}, 3200ms, 800ms);

        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 0, 128, 560ms);
        pbf_move_left_joystick(context, {0, +1}, 5600ms, 800ms);

        // turn towards wall
        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 128, 0, 400ms);
...
void checkpoint_22(
    ...
    [&](size_t attempt_number){         
        ...
        [&](const ProgramInfo& info, VideoStream& stream, ProControllerContext& context){
            realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 0, 70, 240ms);
        });            
        overworld_navigation(env.program_info(), env.console, context, 
...
void checkpoint_23(
    ...
    [&](size_t attempt_number){         
        ...
        // section 1
        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 110, 0, 240ms);
        ...
        handle_unexpected_battles(env.program_info(), env.console, context,
        [&](const ProgramInfo& info, VideoStream& stream, ProControllerContext& context){
            realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 0, 110, 400ms);
        });   
        overworld_navigation(env.program_info(), env.console, context, 
            NavigationStopCondition::STOP_TIME, NavigationMovementMode::DIRECTIONAL_ONLY, 
            128, 15, 12, 12);   
