...
void AutoStory_Checkpoint_36::run_checkpoint(SingleSwitchProgramEnvironment& env, ProControllerContext& context, AutoStoryOptions options, AutoStoryStats& stats) const{
    checkpoint_36(env, context, options.notif_status_update, stats, checkpoint_text());
}
...
void checkpoint_36(
    ...
    [&](size_t attempt_number){         
        ...
        move_cursor_towards_flypoint_and_go_there(env.program_info(), env.console, context, {ZoomChange::ZOOM_IN, -0.219, +1, 640ms});
        ...
        realign_player_from_landmark(
            env.program_info(), env.console, context, 
            {ZoomChange::KEEP_ZOOM, -1, +1, 320ms},
            {ZoomChange::ZOOM_IN, +1, 0, 360ms}
        );
        overworld_navigation(env.program_info(), env.console, context, 
            NavigationStopCondition::STOP_DIALOG, NavigationMovementMode::DIRECTIONAL_ONLY, 
            0, +1, 30, 10, false);
