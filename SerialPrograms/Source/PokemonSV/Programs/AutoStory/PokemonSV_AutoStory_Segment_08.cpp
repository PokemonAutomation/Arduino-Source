...
void checkpoint_13(
    ...
    [&](size_t attempt_number){
        ...
        [&](const ProgramInfo& info, VideoStream& stream, ProControllerContext& context){                  
            ...
            context.wait_for_all_requests();
            realign_player(info, env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 0, 80, 400ms);
...
void checkpoint_14(
    ...
    [&](size_t attempt_number){
        ...
        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 80, 0, 800ms);
        // walk forward so you're off center
        pbf_move_left_joystick(context, {0, +1}, 800ms, 800ms);
        // realign going straight
        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 128, 0, 800ms);
        // walk forward, while still off center
        pbf_move_left_joystick(context, {0, +1}, 16000ms, 800ms);
        // realign diagonally to the right
        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 178, 0, 800ms);
