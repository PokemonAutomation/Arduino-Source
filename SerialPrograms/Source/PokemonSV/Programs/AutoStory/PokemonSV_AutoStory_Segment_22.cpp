...
void AutoStory_Segment_22::run_segment(
    ...
    AutoStory_Checkpoint_50().run_checkpoint(env, context, options, stats);
    AutoStory_Checkpoint_51().run_checkpoint(env, context, options, stats);
    AutoStory_Checkpoint_52().run_checkpoint(env, context, options, stats);
    AutoStory_Checkpoint_53().run_checkpoint(env, context, options, stats);
...
std::string AutoStory_Checkpoint_50::name() const{ return "050 - " + AutoStory_Segment_22().name(); }
...
std::string AutoStory_Checkpoint_53::name() const{ return "053 - " + AutoStory_Segment_22().name(); }
...
void checkpoint_50(
    ...
    [&](size_t attempt_number){
        ...
        context.wait_for_all_requests();
        // set down marker 1
        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 255, 60, 400ms);
        ...
        handle_when_stationary_in_overworld(env.program_info(), env.console, context, 
            ...
            [&](const ProgramInfo& info, VideoStream& stream, ProControllerContext& context){
                pbf_move_left_joystick(context, {-1, 0}, 1024ms, 2040ms);
                realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_OLD_MARKER);
            }
        );
...
void checkpoint_51(
    ...
    [&](size_t attempt_number){
        ...
        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 170, 0, 480ms);
        pbf_move_left_joystick(context, {0, +1}, 14400ms, 800ms);
        realign_player(env.program_info(), env.console, context, PlayerRealignMode::REALIGN_NEW_MARKER, 255, 85, 480ms);
