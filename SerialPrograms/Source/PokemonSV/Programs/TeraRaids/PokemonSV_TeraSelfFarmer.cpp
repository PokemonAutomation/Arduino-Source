...
bool TeraFarmerOpponentFilter::should_battle(size_t stars, const std::string& pokemon) const{
    ...
    std::array<std::string, 8> sixsour{"umbreon", "slowking", "goodra", "mimikyu", "frosmoth", "tinkaton", "toedscruel", "clodsire"};
    std::array<std::string, 10> fivesour{"slowking", "gothitelle", "florges", "goodra", "mimikyu", "altaria", "dragalge", "mismagius", "hatterene", "tinkaton"};
    
    std::array<std::string, 8> sixsweet{"ditto", "jolteon", "talonflame", "dragapult", "maushold", "cyclizar", "kilowattrel", "grafaiai"};
    std::array<std::string, 10> fivesweet{"raichu", "ditto", "delibird", "sableye", "weavile", "froslass", "talonflame", "noivern", "dragapult", "pawmot"};

    std::array<std::string, 10> sixsalty{"pelipper", "torkoal", "hippowdon", "avalugg", "toxapex", "corviknight", "orthworm", "klawf", "garganacl", "dachsbun"};
    std::array<std::string, 11> fivesalty{"slowbro", "cloyster", "bronzong", "hippowdon", "avalugg", "greedent", "corviknight", "coalossal", "copperajah", "orthworm", "garganacl"};
    
    std::array<std::string, 24> sixspicy{"tauros", "gyarados", "flareon", "dragonite", "scizor", "heracross", "tyranitar", "breloom", "salamence", "staraptor", "garchomp", "leafeon", "gallade", "haxorus", "lycanroc", "pincurchin", "revavroom", "baxcalibur", "pawmot", "bombirdier", "mabosstiff", "ceruledge", "kingambit", "annihilape"};
    std::array<std::string, 35> fivespicy{"arcanine", "scyther", "tauros", "gyarados", "dragonite", "scizor", "tyranitar", "breloom", "slaking", "hariyama", "salamence", "staraptor", "luxray", "honchkrow", "garchomp", "abomasnow", "gallade", "krookodile", "haxorus", "braviary", "mudsdale", "tsareena", "passimian", "flapple", "grimmsnarl", "falinks", "indeedee", "revavroom", "baxcalibur", "bombirdier", "mabosstiff", "brambleghast", "ceruledge", "kingambit", "annihilape"};
    
    std::array<std::string, 12> sixbitter{"gengar", "espeon", "gardevoir", "magnezone", "glaceon", "hydreigon", "volcarona", "dragalge", "clawitzer", "sylveon", "glimmora", "armarouge"};
    std::array<std::string, 19> fivebitter{"eevee", "houndoom", "gardevoir", "camerupt", "magnezone", "rotom", "zoroark", "hydreigon", "volcarona", "clawitzer", "oranguru", "appletun", "toxtricity", "polteageist", "pincurchin", "arboliva", "tatsugiri", "glimmora", "armarouge"};
    
    for (const std::string& str : sixall){
        if (SKIP_HERBA && pokemon == str){
            return true;
        }
    };

    for (const std::string& str : fiveall){
        if (SKIP_HERBA && pokemon == str){
             return true;
        }
    };
    ...
    if (SKIP_HERBA && FOCUS_H == Herba::SOUR_H){
        for (const std::string& str : fivesour){
            if (pokemon == str){
                 return true;
            }
        };
        for (const std::string& str : sixsour){
            if (pokemon == str){
                 return true;
            }
        };
    ...
    if (SKIP_HERBA && FOCUS_H == Herba::SWEET_H){
        for (const std::string& str : fivesweet){
            if (pokemon == str){
                 return true;
            }
        };
        for (const std::string& str : sixsweet){
            if (pokemon == str){
                 return true;
            }
        };
    ...
    if (SKIP_HERBA && FOCUS_H == Herba::SALTY_H){
        for (const std::string& str : fivesalty){
            if (pokemon == str){
                 return true;
            }
        };
        for (const std::string& str : sixsalty){
            if (pokemon == str){
                 return true;
            }
        };
    ...
    if (SKIP_HERBA && FOCUS_H == Herba::SPICY_H){
        for (const std::string& str : fivespicy){
            if (pokemon == str){
                 return true;
            }
        };
        for (const std::string& str : sixspicy){
            if (pokemon == str){
                 return true;
            }
        };
    ...
    if (SKIP_HERBA && FOCUS_H == Herba::BITTER_H){
        for (const std::string& str : fivebitter){
            if (pokemon == str){
                 return true;
            }
        };
        for (const std::string& str : sixbitter){
            if (pokemon == str){
                 return true;
            }
        };
