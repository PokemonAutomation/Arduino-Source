...
uint32_t ItemPrinterRNG::calc_num_jobs_with_happiny_dust(
    ...
    int32_t num_happiny_dust_can_use = num_happiny_dust - MIN_HAPPINY_DUST - 50; // give a buffer of 50. signed int to handle negative numbers
    num_happiny_dust_can_use = num_happiny_dust_can_use < 0 ? 0 : num_happiny_dust_can_use;

    uint32_t num_print_jobs = num_happiny_dust_can_use * 0.68; // truncate the float back to int
    env.console.log("Number of Happiny Dust we have: " + std::to_string(num_happiny_dust));
    env.console.log("Number of Happiny Dust we can use (with a buffer of 50): " + std::to_string(num_happiny_dust_can_use));
...
uint32_t ItemPrinterRNG::check_num_happiny_dust(
    ...
    while (true){
        ...
        case 3:{
            ...
            pbf_press_dpad(context, DPAD_RIGHT, 20, 30);
            pbf_press_dpad(context, DPAD_RIGHT, 20, 30);
            detector.find_happiny_dust(env.inference_dispatcher(), env.console, context);
            num_happiny_dust = detector.detect_material_quantity(env.inference_dispatcher(), env.console, context);
