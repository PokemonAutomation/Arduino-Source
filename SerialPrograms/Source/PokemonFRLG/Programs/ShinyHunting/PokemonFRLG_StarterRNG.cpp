...
StarterRNG::StarterRNG()
    ...
    , RESET_TYPE(
        "<b>Reset Type:</b><br>\"Hard\" resets return to the home menu and close the game.<br>\"Soft\" resets don't require returning to the home menu.",
        {
...
void hard_reset(ProControllerContext& context){
    ...
    pbf_press_button(context, BUTTON_A, 100ms, 100ms);
    pbf_press_button(context, BUTTON_HOME, 200ms, 2800ms);
    pbf_press_button(context, BUTTON_A, 200ms, 0ms); // std::chrono::milliseconds(SEED_DELAY - 200));
}

void soft_reset(ProControllerContext& context){
    pbf_press_button(context, BUTTON_B | BUTTON_A | BUTTON_X | BUTTON_Y, 200ms, 0ms); // std::chrono::milliseconds(SEED_DELAY - 200));
}
...
void set_seed_after_delay(ProControllerContext& context, SimpleIntegerOption<uint64_t>& SEED_DELAY){
    ...
    pbf_wait(context, std::chrono::milliseconds(SEED_DELAY - 200)); // subtract 200ms (duration of button push during reset)
    pbf_press_button(context, BUTTON_A, 200ms, 0ms);
}

void load_game_after_delay(ProControllerContext& context, uint64_t& LOAD_DELAY){
    pbf_wait(context, std::chrono::milliseconds(LOAD_DELAY - 200)); // subtract 200ms (duration of button push during reset)
    pbf_press_button(context, BUTTON_A, 200ms, 1300ms);
    // skip recap
    pbf_press_button(context, BUTTON_B, 200ms, 2300ms);
...
void grass_walk_after_delay(ProControllerContext& context, uint64_t& DOUBLE_DELAY){
    ...
    pbf_move_left_joystick(context, {+1, 0}, 33ms, 66ms);
    pbf_move_left_joystick(context, {0, +1}, 33ms, 66ms);
    pbf_move_left_joystick(context, {-1, 0}, 33ms, 66ms);
    pbf_move_left_joystick(context, {0, -1}, 33ms, 66ms);
    pbf_move_left_joystick(context, {+1, 0}, 33ms, 66ms);
    pbf_move_left_joystick(context, {0, +1}, 33ms, 66ms);
    ...
    */
    ...
    while (!shiny_found){
        ...
        if (TARGET == Target::starters){
            ...
            grass_walk_after_delay(context, DOUBLE_DELAY);
            context.wait_for_all_requests();
