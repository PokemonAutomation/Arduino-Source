...
public:
    explicit OperationFailedException(bool send_error_report, Logger& logger, std::string message);
    explicit OperationFailedException(bool send_error_report, Logger& logger, std::string message, std::shared_ptr<const ImageRGB32> screenshot);
    explicit OperationFailedException(bool send_error_report, ConsoleHandle& console, std::string message, bool take_screenshot);
