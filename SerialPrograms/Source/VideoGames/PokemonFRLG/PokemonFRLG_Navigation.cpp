...
#include "VideoGames/PokemonFRLG/Inference/Dialogs/PokemonFRLG_PartyDialogs.h"
#include "VideoGames/PokemonFRLG/Inference/Sounds/PokemonFRLG_CatchFanfareDetector.h"
#include "VideoGames/PokemonFRLG/Inference/Sounds/PokemonFRLG_ShinySoundDetector.h"
#include "VideoGames/PokemonFRLG/Inference/Menus/PokemonFRLG_BagDetector.h"
#include "VideoGames/PokemonFRLG/Inference/Menus/PokemonFRLG_StartMenuDetector.h"
#include "VideoGames/PokemonFRLG/Inference/Menus/PokemonFRLG_LoadMenuDetector.h"
#include "VideoGames/PokemonFRLG/Inference/Menus/PokemonFRLG_SummaryDetector.h"
#include "VideoGames/PokemonFRLG/Inference/Menus/PokemonFRLG_PartyEmptySlotDetector.h"
#include "VideoGames/PokemonFRLG/Inference/Menus/PokemonFRLG_PartyMenuDetector.h"
#include "VideoGames/PokemonFRLG/Inference/Map/PokemonFRLG_MapDetector.h"
#include "VideoGames/PokemonFRLG/Inference/PokemonFRLG_BattlePokemonDetector.h"
#include "VideoGames/PokemonFRLG/Inference/PokemonFRLG_PokedexRegisteredDetector.h"
#include "VideoGames/PokemonFRLG/Programs/PokemonFRLG_BattleMenuNavigation.h"
#include "VideoGames/PokemonFRLG/Programs/PokemonFRLG_SafariOptimalAction.h"
#include "VideoGames/PokemonFRLG/Programs/PokemonFRLG_StartMenuNavigation.h"
...
void exit_battle_after_catch(
    ...
    while (exiting_menus){
        ...
        default:
            ...
            attempts++;
            if (attempts > 3){
                if (in_overworld(console, context)){
                    return;
                }
                OperationFailedExceptionWithScreenshot::fire(
                    ErrorReportMode::SEND_ERROR_REPORT,
    ...
    for (uint64_t i = 0; i <= max_ball_throws; i++){
        ...
        while (true){
            if (current_time() - last_detected_time > std::chrono::seconds(20)){
                console.log("auto_catch(): no battle activity detected for 20 seconds");
                return -1;
            }
            ...
            int ret = run_until<ProControllerContext>(
                ...
                { battle_menu, party_menu, black_screen, dex_registration, catch_detector },
                10ms
            );
            ...
            case 1:
                ...
                context.wait_for_all_requests();
                continue;
            case 2: // either failed catch or fainting -> party menu
                pbf_wait(context, 200ms);
                continue;
            case 3:
            case 4:
                console.log("Pokemon Caught!", COLOR_BLUE);
                exit_battle_after_catch(console, context);
                return static_cast<int>(i);
            default:
                console.log("No recognized state. Try checking if in the overworld...");
                if (in_overworld(console, context)){
                    console.log("Overworld detected.");
                    return 0; // no catch detected
                }
            }
            break;
        }
