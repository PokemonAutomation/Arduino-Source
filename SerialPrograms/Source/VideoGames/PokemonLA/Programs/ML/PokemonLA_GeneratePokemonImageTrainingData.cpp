...
#include "VideoGames/PokemonLA/Inference/Objects/PokemonLA_ButtonDetector.h"
#include "VideoGames/PokemonLA/Programs/PokemonLA_GameEntry.h"
#include "VideoGames/PokemonLA/Programs/PokemonLA_RegionNavigation.h"
...
void GeneratePokemonImageTrainingData::select_starting_location(SingleSwitchProgramEnvironment& env, ProControllerContext& context){
    ...
    case StartingLocation::JUBILIFE_VILLAGE:
        ...
        pbf_move_left_joystick(context, {+1, -0.173}, 960ms, 480ms); // move to right
        pbf_press_button(context, BUTTON_ZL, 240ms, 1000ms); // rotate camera to point to where the character is facing
        pbf_move_left_joystick(context, {0, +1}, 5600ms, 240ms); // move forward
        pbf_move_left_joystick(context, {+1, +0.023}, 240ms, 480ms); // move to right
        break;
    case StartingLocation::FIELDLANDS_FIELDLANDS:
        env.log("Starting at Fieldlands - Fieldlands Camp");
        pbf_move_left_joystick(context, {-0.609, +1}, 1040ms, 80ms);
        break;
    case StartingLocation::FIELDLANDS_HEIGHTS:
        env.log("Starting at Fieldlands - Heights Camp");
        pbf_move_left_joystick(context, {-0.688, +1}, 1320ms, 80ms);
        break;
    case StartingLocation::MIRELANDS_MIRELANDS:
        env.log("Starting at Mirelands - Mirelands Camp");
        pbf_move_left_joystick(context, {+0.567, +1}, 1120ms, 80ms);
        break;
    case StartingLocation::MIRELANDS_BOGBOUND:
        env.log("Starting at Mirelands - Bogbound Camp");
        pbf_move_left_joystick(context, {-1, +0.57}, 1720ms, 80ms);
        break;
    case StartingLocation::COASTLANDS_BEACHSIDE:
        env.log("Starting at Coastlands - Beachside Camp");
        pbf_move_left_joystick(context, {-1, +0.727}, 1280ms, 80ms);
        break;
    case StartingLocation::COASTLANDS_COASTLANDS:
        env.log("Starting at Coastlands - Coastlands Camp");
        pbf_move_left_joystick(context, {-1, +0.688}, 1440ms, 80ms);
        break;
    case StartingLocation::HIGHLANDS_HIGHLANDS:
        env.log("Starting at Highlands - Highlands Camp");
        pbf_move_left_joystick(context, {-1, +0.844}, 1120ms, 80ms);
        break;
    case StartingLocation::HIGHLANDS_MOUNTAIN:
        env.log("Starting at Highlands - Mountain Camp");
        pbf_move_left_joystick(context, {-1, +0.883}, 800ms, 80ms);
        break;
    case StartingLocation::HIGHLANDS_SUMMIT:
        env.log("Starting at Highlands - Summit Camp");
        pbf_move_left_joystick(context, {-0.922, +1}, 1200ms, 80ms);
        break;
    case StartingLocation::ICELANDS_SNOWFIELDS:
    ...
    case StartingLocation::ICELANDS_ICEPEAK:
        ...
        pbf_move_left_joystick(context, {-0.844, +1}, 1440ms, 80ms);
        break;
    }
