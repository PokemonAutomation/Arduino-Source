    ...
    while(true){
        ...
        if (SHINY_CHECK_RUN_TIME.get() > Milliseconds(0)){
            ...
            if (FORWARD_RUN_ROLL_TIME.get() == Milliseconds(0)){
                ...
                pbf_controller_state(context, BUTTON_NONE, DPAD_NONE, {0, +1}, {0, 0}, FORWARD_RUN_ROLL_TIME);
                pbf_controller_state(context, BUTTON_Y, DPAD_NONE, {0, +1}, {0, 0}, FORWARD_RUN_TIME.get() + SHINY_CHECK_RUN_TIME.get() - FORWARD_RUN_ROLL_TIME.get());
            }
            pbf_wait(context, 1000ms); // wait one sec to listen to shiny sound
            context.wait_for_all_requests();
        }
        // No sound detected so far. Reset game
...
void ShinyHunt_HyperspaceHunter::program(SingleSwitchProgramEnvironment& env, ProControllerContext& context){
    ...
    run_until<ProControllerContext>(
        env.console, context,
        [&](ProControllerContext& context){
            // Separate logic based on hunt method
