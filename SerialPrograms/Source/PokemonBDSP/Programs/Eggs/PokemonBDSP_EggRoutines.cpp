...
void egg_spin(ProControllerContext& context, bool press_A, Milliseconds duration){
    for (Milliseconds c = 0ms; c < duration; c += 56ms * 4 + 32ms + 48ms + 64ms + 80ms){
        if (press_A){
            ssf_press_button(context, BUTTON_ZL, 0ms, 80ms);
        }
        pbf_move_left_joystick(context, {-1, +1}, 56ms, 0ms);
        pbf_move_left_joystick(context, {0, +1}, 56ms, 0ms);
        pbf_move_left_joystick(context, {+1, +1}, 56ms, 0ms);
        pbf_move_left_joystick(context, {+1, 0}, 32ms, 0ms);
        pbf_move_left_joystick(context, {+1, -1}, 56ms, 0ms);
        pbf_move_left_joystick(context, {0, -1}, 48ms, 0ms);
        pbf_move_left_joystick(context, {-1, -1}, 64ms, 0ms);
        pbf_move_left_joystick(context, {-1, 0}, 80ms, 0ms);
    }
}
