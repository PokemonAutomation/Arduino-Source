...
#endif
        pbf_controller_state(context, BUTTON_A, DPAD_NONE, 128, 0, 128, 128, 5);
...
void pickup_column(const BotBaseContext& context){
    ...
    pbf_move_right_joystick(context, 128, 255, 10, BOX_SCROLL_DELAY);
    pbf_move_right_joystick(context, 128, 255, 10, BOX_SCROLL_DELAY);
    pbf_press_button(context, BUTTON_A, 10, GameSettings::instance().BOX_PICKUP_DROP_DELAY);
}
void party_to_column(const BotBaseContext& context, uint8_t column){
    const uint16_t BOX_SCROLL_DELAY = GameSettings::instance().BOX_SCROLL_DELAY_0;
    pbf_move_right_joystick(context, 128, 0, 10, BOX_SCROLL_DELAY);
    if (column < 3){
...
//    pbf_press_button(context, BUTTON_A, 10, GameSettings::instance().MENU_TO_POKEMON_DELAY);
    ...
    pbf_press_button(context, BUTTON_R, 10, GameSettings::instance().POKEMON_TO_BOX_DELAY0);
}
void box_to_overworld(const BotBaseContext& context){
    pbf_press_button(context, BUTTON_B, 10, GameSettings::instance().BOX_TO_POKEMON_DELAY);
    pbf_press_button(context, BUTTON_B, 10, GameSettings::instance().POKEMON_TO_MENU_DELAY);
    pbf_press_button(context, BUTTON_B, 10, GameSettings::instance().MENU_TO_OVERWORLD_DELAY);
}
void withdraw_1st_column_from_overworld(const BotBaseContext& context){
    ...
    pbf_move_right_joystick(context, 0, 128, 10, BOX_SCROLL_DELAY);
    pbf_move_right_joystick(context, 128, 255, 10, BOX_SCROLL_DELAY);
    pbf_press_button(context, BUTTON_A, 10, BOX_PICKUP_DROP_DELAY);
...
void swap_party(const BotBaseContext& context, uint8_t current_column){
    ...
    if (current_column < 5){
        ...
        pbf_press_button(context, BUTTON_R, 10, GameSettings::instance().BOX_CHANGE_DELAY_0);
        pbf_move_right_joystick(context, 255, 128, 10, BOX_SCROLL_DELAY);
        pickup_column(context);
