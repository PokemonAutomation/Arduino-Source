...
#ifndef PokemonAutomation_Kernels_SparseBinaryMatrixBase_H
#define PokemonAutomation_Kernels_SparseBinaryMatrixBase_H
...
class SparseBinaryMatrixBase{
public:
    using Tile = TileType;

public:
    //  Rule of 5
    ~SparseBinaryMatrixBase();
    SparseBinaryMatrixBase(SparseBinaryMatrixBase&& x);
    void operator=(SparseBinaryMatrixBase&& x);
    SparseBinaryMatrixBase(const SparseBinaryMatrixBase& x);
    void operator=(const SparseBinaryMatrixBase& x);

public:
    //  Construction
    SparseBinaryMatrixBase();
    SparseBinaryMatrixBase(size_t width, size_t height);
...
public:
    ...
    PackedBinaryMatrixBase<TileType> submatrix(size_t x, size_t y, size_t width, size_t height) const;
...
const Tile& SparseBinaryMatrixBase<Tile>::tile(TileIndex index) const{
...
template <typename Tile> PA_FORCE_INLINE
Tile& SparseBinaryMatrixBase<Tile>::tile(TileIndex index){
    return m_data[index];
}
template <typename Tile> PA_FORCE_INLINE
const Tile& SparseBinaryMatrixBase<Tile>::tile(size_t x, size_t y) const{
    return tile(TileIndex(x, y));
}
template <typename Tile> PA_FORCE_INLINE
Tile& SparseBinaryMatrixBase<Tile>::tile(size_t x, size_t y){
    return tile(TileIndex(x, y));
}
...
template <typename Tile> PA_FORCE_INLINE
uint64_t SparseBinaryMatrixBase<Tile>::word64(size_t x, size_t y) const{
    static_assert(TILE_WIDTH == 64);
    const Tile& tile = this->tile(x, y / TILE_HEIGHT);
    return tile.row(y % TILE_HEIGHT);
}
template <typename Tile> PA_FORCE_INLINE
uint64_t& SparseBinaryMatrixBase<Tile>::word64(size_t x, size_t y){
    static_assert(TILE_WIDTH == 64);
...
template <typename Tile>
bool SparseBinaryMatrixBase<Tile>::get(size_t x, size_t y) const{
    const Tile& tile = this->tile(x / TILE_WIDTH, y / TILE_HEIGHT);
    return tile.get_bit(x % TILE_WIDTH, y % TILE_HEIGHT);
}
template <typename Tile>
void SparseBinaryMatrixBase<Tile>::set(size_t x, size_t y, bool set){
