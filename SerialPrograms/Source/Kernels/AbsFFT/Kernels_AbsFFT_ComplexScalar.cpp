...
void reduce4_scalar(const TwiddleTable& table, int k, float* T){
    ...
    TableRowReader w1(table[k - 1].w1.data());
    TableRowReader w2(table[k].w1.data());
    TableRowReader w3(table[k].w3.data());
    size_t c = 0;
    do{
        float r0, r1, r2, r3;
        float i0, i1, i2, i3;
...
void fft_complex_t1_scalar(float T[4]){
    float r0, r1;
    float i0, i1;
    r0 = T[0];
    i0 = T[1];
...
void fft_complex_t2_scalar(float T[8]){
    float r0, r1, r2, r3;
    float i0, i1, i2, i3;
    r0 = T[0];
    i0 = T[1];
...
void fft_complex_t3_scalar(float T[16]){
    {
        float r0, r1, r2, r3;
        float i0, i1, i2, i3;
        r0 = T[ 0];
...
void fft_complex_tk_scalar(const TwiddleTable& table, int k, float* T){
    ...
    reduce4_scalar(table, k, T);

    k -= 2;
    size_t stride = (size_t)2 << k;
    fft_complex_tk_scalar(table, k, T + 0*stride);
    fft_complex_tk_scalar(table, k, T + 1*stride);
    fft_complex_tk_scalar(table, k, T + 2*stride);
    fft_complex_tk_scalar(table, k, T + 3*stride);
}
