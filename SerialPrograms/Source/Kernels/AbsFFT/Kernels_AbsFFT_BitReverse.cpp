...
void interleave_f32_scalar(int len_k, float* out, const float* in){
...
void interleave_f32(int len_k, float* out, const float* in){
    if ((size_t)len_k < VECTOR_K + 1){
        interleave_f32_scalar(len_k, (float*)out, (const float*)in);
        return;
    }
    size_t vstride = (size_t)1 << (len_k - 1 - VECTOR_K);
...
PA_FORCE_INLINE void interleave_u64_scalar(int len_k, uint64_t* out, const uint64_t* in){
    size_t stride = (size_t)1 << (len_k - 1);
...
PA_FORCE_INLINE void interleave_u64(int len_k, uint64_t* out, const uint64_t* in){
    if ((size_t)len_k < VECTOR_K || VECTOR_K == 0){
        interleave_u64_scalar(len_k, out, in);
        return;
    }
    size_t vstride = (size_t)1 << (len_k - VECTOR_K);
    const vtype* in0 = (const vtype*)in;
    const vtype* in1 = in0 + vstride;
    vtype* out0 = (vtype*)out;
    size_t lc = vstride;
...
void bitreverse_u64_ip(int len_k, uint64_t* data, uint64_t* temp){
    if (len_k <= 1){
        return;
    }
    if (len_k == 2){
...
void bitreverse_u64_np(int len_k, uint64_t* out, uint64_t* in){
    if (len_k == 0){
        out[0] = in[0];
        return;
    }
