...
public:
    ToBlackWhite_RgbRange_arm64_NEON(uint32_t mins, uint32_t maxs, bool in_range_black)
        ...
        , m_count_u32(vdupq_n_u32(0))
    {}
    ToBlackWhite_RgbRange_arm64_NEON(ToBlackWhiteRgb32RangeFilter& filter)
        : ToBlackWhite_RgbRange_arm64_NEON(filter.mins, filter.maxs, filter.in_range_black)
    {}
...
void to_blackwhite_rgb32_range_arm64_NEON(
    ...
    return filter_per_pixel<ToBlackWhite_RgbRange_arm64_NEON>(
        image, bytes_per_row, width, height, filter, filter_count
    );
}
