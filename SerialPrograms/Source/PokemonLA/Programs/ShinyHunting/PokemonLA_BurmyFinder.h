...
private:
    ...
    void run_iteration(SingleSwitchProgramEnvironment& env, ControllerContext& context, TreeCounter& tree_counter);
    ...
    bool check_tree(SingleSwitchProgramEnvironment& env, ControllerContext& context);
    ...
    void check_tree_no_stop(SingleSwitchProgramEnvironment& env, ControllerContext& context);
    ...
    bool handle_battle(SingleSwitchProgramEnvironment& env, ControllerContext& context);
    ...
    void disable_shiny_sound(ControllerContext& context);
    // Re-enable shiny sound detection.
    // Also call `context.wait_for_all_requests()` to make sure controller commands are all executed.
    // See `m_enable_shiny_sound` for more context.
    void enable_shiny_sound(ControllerContext& context);
    ...
    void go_to_height_camp(SingleSwitchProgramEnvironment& env, ControllerContext& context);
    ...
    size_t grouped_path(SingleSwitchProgramEnvironment& env, ControllerContext& context, size_t path, TreeCounter& tree_counter);

    // Check trees individually going from camp based on the last tree checked for each path
    void single_path(SingleSwitchProgramEnvironment& env, ControllerContext& context, size_t path, size_t last_tree, TreeCounter& tree_counter);
